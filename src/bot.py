...
async def on_command_error(ctx: Context, err: Exception) -> None:
    ...
    if settings.THROW_COMMAND_ERRORS:
        ...
        as_embed = False#await server_settings.get_use_embeds(ctx)
        try:
...
@cooldown(rate=RATE, per=COOLDOWN * 2, type=BucketType.user)
async def cmd_targets_top(ctx: Context, division: str, count: int = None, fleet_count: int = None) -> None:
    """
    ...
    Parameters:
      ...
      fleet_count:    Optional. The maximum fleets to be considered. Accepts a range.
    ...
    Examples:
      ...
      /targets top a 5 4 - Print up to 5 players per fleet of Division A in 4 fleets.
     

    Notes:
      The parameter 'count' is constrained depending on the division:
        Division A: Default 20
        Division B: Default 15
        Division C: Default 10
        Division D: Default 10
        
    The parameter 'fleet count' is constrained depending on the division:
        Division A: max 8
        Division B: max 12
        Division C: max 15
        Division D: max 15
    ...
    max_fleet_count = lookups.DIVISION_MAX_FLEET_COUNT_TARGETS_TOP[division_design_id]
    if fleet_count:
        if fleet_count < 0:
            raise ValueError('The fleet count must not be a negative number.')
        elif fleet_count > max_fleet_count:
            raise ValueError(f'The maximum fleet count to be displayed for division {division.upper()} is {max_count}.')
    else:
        fleet_count = max_fleet_count
    # criteria_lines, min_star_value, max_star_value, min_trophies_value, max_trophies_value, max_highest_trophies = pss_top.get_targets_parameters(star_value, trophies, max_highest_trophies)
    ...
    if yesterday_tourney_data:
        yesterday_user_infos = pss_top.filter_targets(yesterday_tourney_data.users.values(), division_design_id, last_month_user_data, current_fleet_data)
        if not yesterday_user_infos:
